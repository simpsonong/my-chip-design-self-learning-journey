...
single_port_ram_synchronous dut_single_port_ram_synchronous (.clk(TICK), .we(WE), .addr(ADDR), .data_in(DATA_IN), .data_out(DATA_OUT0), .out_mem(DATA_OUT));
...
if(j>1 && j<5'b10000)begin
//NOTHING
end

else begin

if(input_coverage[j-5'b10000] == 0) begin
$display ("input WE = %b, ADDR = %b has occured %d times ***ERROR***", j[4], j[3:0], input_coverage[j-5'b10000]);
ERRORS = ERRORS + 1;
end


else begin
$display ("input WE = %b, ADDR = %b has occured %d times", j[4], j[3:0], input_coverage[j-5'b10000]);
end
end

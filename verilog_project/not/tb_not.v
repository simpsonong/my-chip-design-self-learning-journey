`timescale 1ns/10ps

module tb_not;

//VECTORS FOR DUT
reg A;
wire Q_gate, Q_data, Q_beh;

//VECTORS FOR TESTING

reg TICK;
integer FD, COUNT;
reg [8*9-1:0] COMMENT;
reg QEXPECTED;
reg [31:0] VECTORCOUNT, ERRORS;
int i;
reg [31:0] input_coverage[3:0];
reg eof;
reg prev_A;
reg prev_QEx;
reg prev_QExbar;


//DUT INSTANTIATION

not_gate dut_not_gate (.a(A), .q(Q_gate));
not_dataflow dut_not_dataflow (.a(A), .q(Q_data));
not_behavioral dut_not_behavioral (.a(A), .q(Q_beh));



//DUMP EVERYTHING INTO DUMPFILE

initial begin

$dumpfile ("tb_not.vcd");
$dumpvars (0, tb_not);

end

//TICKPERIOD

localparam TICKPERIOD = 20;

always begin

#(TICKPERIOD/2) TICK = ~TICK;

end



//TASKS

//task 1 initialize

task initialize; begin

FD = $fopen ("tb_not.tv" , "r");
COUNT = $fscanf (FD, "%s", COMMENT);

COUNT = $fscanf (FD, "%s %b %b", COMMENT, A, QEXPECTED);
TICK = 0;
VECTORCOUNT = 0;
ERRORS = 0;

for ( i = 0 ; i < 4 ; i++ ) begin
input_coverage[i] = 0;
end

eof = 0;

$display ();
$display ("TEST_START------------------------------------------------------------------------------------------------------------------------");
$display ();
$display ("                     | TIME | A |  Q_gate  |  Q_data  |  Q_beh  | QEXPECTED |");
$display ("----------------------------------------------------------------------------------------------------------------------------------");



end
endtask



//task 2 scan_file

task scan_file; begin


COUNT = $fscanf (FD,"%s %b %b", COMMENT, A, QEXPECTED);
eof = (COUNT == -1);

end
endtask

//task 3 random_in

task random_in; begin


A =  $urandom %5 ;

case (A)
1'b0: COMMENT = "HIGH";
1'b1: COMMENT = "LOW";

endcase

#($urandom_range (0.1,10));

end
endtask

//task 4 QEXP

task QEXP; begin

case (A)

 1'b0: begin
QEXPECTED = 1'b1;
end
 1'b1: begin
QEXPECTED = 1'b0;
end
 
endcase



end
endtask



//task 5 close

task close; begin


#10;
$fclose (FD);

$display ();
$display ("COVERAGE_REPORT");

for ( i = 0; i < 4; i++ ) begin

if(input_coverage[i] == 0) begin

$display ("input %b to input %b has occured %d times (***ERROR***)",i[1], i[0], input_coverage[i]);
ERRORS = ERRORS +1;

end

else begin

$display ("input %b to input %b has occured %d times",i[1], i[0], input_coverage[i]);

end

end

$display ();
$display ("VECTORCOUNT = %d", VECTORCOUNT);
$display ("ERRORS = %15d", ERRORS);
$display ();
$display ("TEST_END--------------------------------------------------------------------------------------------------------------------------");
$display ();

$finish;

end
endtask

//task 6 vectorcount

task vectorcount; begin

VECTORCOUNT = VECTORCOUNT + 1;

end
endtask


//task 7 display_file

task display_file; begin

$display ("%10d %9s | %4d | %1d | %4d     | %4d     | %4d    | %5d     |", VECTORCOUNT, COMMENT, $time, A, Q_gate, Q_data, Q_beh, QEXPECTED);

end
endtask

//task 8 coverage_update

task coverage_update; begin

input_coverage[{prev_A, A}]++;

end
endtask

//task 10 errors_check

task errors_check; begin


if (Q_gate !== QEXPECTED) begin

$display ("***ERROR: Q_gate = %b, Q_expected = %b", Q_gate, QEXPECTED);
ERRORS = ERRORS + 1;

end

if (Q_data !== QEXPECTED) begin

$display ("***ERROR: Q_dataflow = %b, Q_expected = %b", Q_data, QEXPECTED);
ERRORS = ERRORS + 1;

end

if (Q_beh !== QEXPECTED) begin

$display ("***ERROR: Q_behavioral = %b, Q_expected = %b", Q_beh, QEXPECTED);
ERRORS = ERRORS + 1;

end


end
endtask



//DRIVE

//0 initialize

initial begin

initialize;

end


//2 scan file on negedge TICK

always @ (negedge TICK) begin


#1.2;

if (!eof) begin



scan_file;

end
end



//3 check file on posedge TICK

always  @ (posedge TICK) begin
coverage_update;
prev_A = A;

#0.1;

QEXP;
display_file;
errors_check;
vectorcount;

#0.02;

prev_QEx = QEXPECTED;



end



// eof

initial begin

wait (eof);

$display ();
$display("%13s ***time = %0d,eof ***","",$time);
$display ();

for (i = 0; i < 50; i++) begin


if ( A == 2 )
begin

vectorcount;
close;

end

else begin

random_in;

end
end


close;

end


endmodule

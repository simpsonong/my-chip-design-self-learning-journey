...
module tb_dual_port_ram_synchronous;
...
reg [DATA_WIDTH-1:0]DATA_OUT_A_EXPECTED1,DATA_OUT_B_EXPECTED1;
...
dual_port_ram_synchronous dut_dual_port_ram_synchronous (.clk(TICK), .we_a(WE_A), .addr_a(ADDR_A), .data_in_a(DATA_IN_A), .data_out_a(DATA_OUT_A), .we_b(WE_B), .addr_b(ADDR_B), .data_in_b(DATA_IN_B), .data_out_b(DATA_OUT_B));
...
$dumpfile ("tb_dual_port_ram_synchronous.vcd");
$dumpvars (0, tb_dual_port_ram_synchronous);
...
FD = $fopen ("tb_dual_port_ram_synchronous.tv" , "r");
...
$display ("TEST_START--------------------------------------------------------------------------------------------------------------------------------------------------");
$display ();
$display ("                     | TIME | WE_A | ADDR_A | DATA_IN_A | DATA_OUT_A | DATA_OUT_A_EXPECTED | WE_B | ADDR_B | DATA_IN_B | DATA_OUT_B | DATA_OUT_B_EXPECTED |");
$display ("------------------------------------------------------------------------------------------------------------------------------------------------------------");
...
$display ("input WE_A = %b, ADDR_A = %b has occured %d times ***ERROR***", j[4], j[3:0], input_coverage_a[j]);
ERRORS = ERRORS + 1;
end


else begin
$display ("input WE_A = %b, ADDR_A = %b has occured %d times", j[4], j[3:0], input_coverage_a[j]);
end
end
...
$display ("input WE_B = %b, ADDR_B = %b has occured %d times ***ERROR***", j[4], j[3:0], input_coverage_b[j]);
ERRORS = ERRORS + 1;
end


else begin
$display ("input WE_B = %b, ADDR_B = %b has occured %d times", j[4], j[3:0], input_coverage_b[j]);
end
end
...
$display ("%10d %9s | %4d | %2b   | %5b  | %5c     | %5c      | %10c          | %2b   | %5b  | %5c     | %5c      | %10c          |", VECTORCOUNT, COMMENT, $time, WE_A, ADDR_A, DATA_IN_A, DATA_OUT_A, DATA_OUT_A_EXPECTED, WE_B, ADDR_B, DATA_IN_B, DATA_OUT_B, DATA_OUT_B_EXPECTED);
...
if ({WE_A,ADDR_A} !== prev_INPUT_A) begin
...
if ({WE_B,ADDR_B} !== prev_INPUT_B) begin
...
if (WE_A == 1'b0 && ADDR_A == 5'b10000 || WE_B == 1'b0 && ADDR_B == 5'b10000)
begin

...
reg prev_en[0:3];
reg flag_init;
wire flag_general,flag_mantain,flag_freq;
reg [4:0]c;
wire [4:0]start_test;
...
input_register dut_input_register (.TICKPERIOD(TICKPERIOD), .TICK(TICK), .LD_bar(LD_bar), .CLR_bar(CLR_bar), .D(D), .eof(eof), .done(done), .flag_init(flag_init),
                                   .flag_general(flag_general), .flag_mantain(flag_mantain), .flag_freq(flag_freq),

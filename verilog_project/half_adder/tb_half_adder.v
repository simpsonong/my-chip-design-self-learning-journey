...
half_adder_gate dut_half_adder_gate (.a(A), .b(B), .sum(SUM_gate), .cout(COUT_gate));
half_adder_dataflow dut_half_adder_dataflow (.a(A), .b(B), .sum(SUM_data), .cout(COUT_data));
half_adder_behavioral dut_half_adder_behavioral (.a(A), .b(B), .sum(SUM_beh), .cout(COUT_beh));
...
FD = $fopen ("tb_half_adder.tv" , "r");
COUNT = $fscanf (FD, "%s", COMMENT);

COUNT = $fscanf (FD, "%s %b %b %b %b", COMMENT, A, B, SUM_EXPECTED, COUT_EXPECTED);
TICK = 0;
VECTORCOUNT = 0;
ERRORS = 0;
...
$display ("TEST_START-------------------------------------------------------------------------------------------------------------------------------------");
$display ();
$display ("                     | TIME | A | B | COUT_gate | SUM_gate | COUT_data | SUM_data | COUT_beh | SUM_beh | COUT_EXPECTED | SUM_EXPECTED |");
$display ("-----------------------------------------------------------------------------------------------------------------------------------------------");
...
$display ("input A = %b & B = %b has occured %d times (***ERROR***)", i[1], i[0], input_coverage[i]);
ERRORS = ERRORS +1;
...
$display ("input A = %b & B = %b has occured %d times", i[1], i[0], input_coverage[i]);
...
$display ("%10d %9s | %4d | %1d | %1d | %5d     | %5d    | %5d     | %5d    | %4d     | %4d    | %7d       | %6d       |", VECTORCOUNT, COMMENT, $time, A, B, COUT_gate, SUM_gate, COUT_data, SUM_data, COUT_beh, SUM_beh, COUT_EXPECTED, SUM_EXPECTED);
